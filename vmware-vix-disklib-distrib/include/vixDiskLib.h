...
typedef struct {
   ...
   char *sslCertificate;         // PEM-encoded certificate
...
typedef struct {
   VixDiskLibSectorType startOffset;   // Start position
} VixDiskLibQueryChangedAreaParams;

typedef struct {
   VixDiskLibSectorType startOffset; // Start position of changed areas range
   VixDiskLibSectorType length;      // Length of changed areas range
   VixDiskLibBlockList *areas;       // Array of changed areas
} VixDiskLibDiskChangeInfo;
...
typedef struct {
   char *path;                      // VMDK file name given as absolute path
   uint32 flags;                    // Bitwise or'ed combination of
                                    // VIXDISKLIB_FLAG_OPEN_XXX flags
   char *changeId;                  // Change id
} VixDiskLibOpenParams;
...
/**
 * Return the connection from disk handle.
 * @param diskHandle [in] Handle to an open virtual disk.
 * @param connection [out] The VixDiskLib connection.
 * @return VIX_OK if success, suitable VIX error code otherwise.
 */
VixError
VixDiskLib_GetConnectionFromDisk(VixDiskLibHandle diskHandle,
                                 VixDiskLibConnection *connection);
 ...
 * Opens a local or remote virtual disk.
 * @param connection [in] A valid connection.
 * @param openParams [in] VixDiskLibOpenParams.
 * @param diskHandle [out] Handle to opened disk, NULL if disk was not opened.
 * @return VIX_OK if success, suitable VIX error code otherwise.
 */
VixError
VixDiskLib_OpenEx(const VixDiskLibConnection connection,
                  const VixDiskLibOpenParams *openParams,
                  VixDiskLibHandle *diskHandle);

/**
 * Get disk changed areas according to the queryParams.
 * @param diskHandle [in] Handle to an open virtual disk.
 * @param queryParams [in] Query parameters.
 * @param changeList [out] Disk changed areas.
 * @return VIX_OK if success, suitable error otherwise.
 */
VixError VixDiskLib_QueryChangedAreas(VixDiskLibHandle diskHandle,
                       const VixDiskLibQueryChangedAreaParams *queryParams,
                       VixDiskLibDiskChangeInfo **changeList);

/**
 * Frees the changeList allocated by VixDiskLib_QueryChangedAreas.
 * @param changeList [in] Disk changed areas to be freed.
 * @return None.
 */
void VixDiskLib_FreeChangedAreas(VixDiskLibDiskChangeInfo *changeList);
